...
export function EduPiStudentFacts({ factSpine, studentId }: { factSpine: EducationFactSpine | null; studentId: string }) {
  ...
  const view = factSpine.studentViews.find(item => item.studentId === studentId);
  const facts = new Map([...factSpine.acceptedFacts, ...factSpine.factCandidates].map(item => [item.id, item]));
  const rows = [...(view?.acceptedFactIds || []), ...(view?.pendingFactIds || [])].flatMap(id => facts.get(id) || []);
  ...
  return <details className="edupi-student-core-facts" open>
    ...
    <div>{rows.map(fact => {
      ...
      return <article key={fact.id}>
        ...
        <details><summary>来源 {Math.max(sources.length, fact.sourceIds.length)}</summary>{sources.length ? sources.map(source => <div key={source.id}><strong>{sourceLabels[source.sourceKind] || source.sourceKind}</strong><p>{source.text}</p><time>{new Date(source.observedAt).toLocaleString("zh-CN")}</time></div>) : fact.sourceIds.map(sourceId => <code key={sourceId}>{sourceId}</code>)}</details>
      </article>;
