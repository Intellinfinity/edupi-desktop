...
import { isTaskReviewable, taskWorkStatusLabel, workCaseForTask } from "@/lib/edupi-work-case";
import { EduPiTimetableGrid } from "./EduPiTimetableGrid";
import { matchesTaskQuery, taskCategory } from "@/lib/edupi-task-category";
import { EduPiPreparationStatus } from "./EduPiPreparationStatus";
import { EduPiTeachingFacts } from "./EduPiTeachingFacts";
import { EduPiTeachingPriorities } from "./EduPiTeachingPriorities";
...
export function EduPiTeachingWorkspace({ data, context, query, selectedObjectId, onObject, onTask, onNavigate, onStartAgent, onCalendarSelection, onEducation, onDeleteEntity }: { data: EducationContract; context: TeacherContextSnapshot | null; query: string; selectedObjectId: string | null; onObject: (id: string) => void; onTask: (task: TeacherTask) => void; onNavigate: (view: "calendar" | "tasks" | "memory", objectId?: string) => void; onStartAgent: (prompt: string, mode?: "insert" | "replace") => void; onCalendarSelection: (selection: CalendarItemSelection) => void; onEducation: (data: EducationContract) => void; onDeleteEntity: (kind: EducationEntityDeleteKind, id: string, label: string) => Promise<boolean> }) {
  ...
  const activePriorities = data.continuity.teachingPriorities.filter((priority) => priority.status === "active" && (!query || `${priority.subject} ${priority.className || ""} ${priority.topic} ${priority.note || ""}`.toLocaleLowerCase().includes(query.toLocaleLowerCase())));
  ...
  const tasks = data.tasks.filter((task) => taskCategory(task) === "teaching" && matchesTaskQuery(task, query));
  ...
  const openNextPreparation = () => nextTeachingTask ? onTask(nextTeachingTask) : onStartAgent(`下一节课：${nextSubject}`, "replace");
  ...
  return <main className="edupi-module-workspace edupi-teaching-workspace">{header}<section className="edupi-teaching-next-line"><div><span>下一节课</span><strong>{nextSubject}</strong><p>{nextTeachingTask?.sourceEventDate || (nextSlot ? `周${String(nextSlot.day_of_week)} · 第 ${String(nextSlot.period)} 节${nextSlot.class_name ? ` · ${String(nextSlot.class_name)}` : ""}` : "课表待导入")} {nextTeachingTask ? `· ${taskWorkStatusLabel(nextTeachingTask, nextWorkCase)}` : ""}</p></div><button type="button" onClick={nextTeachingTask ? openNextPreparation : () => onObject("teaching:schedule")}>{nextTeachingTask ? "查看准备" : "课程表"}</button></section><section className="edupi-teaching-preparation"><header><h2>本周课前准备</h2><span>{weekPreparations.length} 节{preparedCount > 0 ? ` · ${preparedCount} 已准备` : ""}</span></header><div>{weekPreparations.map((task) => { const workCase = workCaseForTask(data, task.id); return <button type="button" key={taskKey(task)} onClick={() => onTask(task)}><time>{task.sourceEventDate?.slice(5).replace("-", "/") || "--/--"}</time><span><strong>{task.sourceEventName || taskDisplayTitle(task)}</strong><small>{teachingPreparationSummary(task)}</small></span><em className={`is-${workCase?.currentState || "planned"}`}><i className={`edupi-flow-state is-${workCase?.currentState || "planned"}`} aria-hidden="true" />{taskWorkStatusLabel(task, workCase)}</em></button>; })}{weekPreparations.length === 0 ? <p>本周课前工作包尚未形成</p> : null}</div></section><section className="edupi-teaching-home-section"><header><h2>本周课程</h2><button type="button" onClick={() => onObject("teaching:schedule")}>完整课程表</button></header><EduPiTimetableGrid slots={slots} onSelect={selectCourse} compact /></section><div className="edupi-teaching-home-columns"><section><header><h2>当前教学重点</h2><button type="button" onClick={() => onObject("teaching:knowledge")}>全部</button></header>{activePriorities.length > 0 ? activePriorities.slice(0, 4).map((item) => <details key={item.id}><summary><strong>{item.topic}</strong><span>{item.note || "教师维护"}</span></summary><p>{item.subject} · {item.className || "全部班级"}</p></details>) : knowledge.slice(0, 4).map((item) => <details key={item.id}><summary><strong>{item.topic}</strong><span>{item.commonErrors[0]?.description || "暂无共性问题"}</span></summary><p>{item.strugglingStudents.length ? `需关注：${item.strugglingStudents.join("、")}` : "暂无需特别关注学生"}</p></details>)}</section><section><header><h2>备课任务</h2><button type="button" onClick={() => onObject("teaching:tasks")}>全部</button></header>{tasks.slice(0, 5).map((task) => { const workCase = workCaseForTask(data, task.id); return <button type="button" key={taskKey(task)} onClick={() => onTask(task)}><strong>{taskDisplayTitle(task)}</strong><span>{workCase ? taskWorkStatusLabel(task, workCase) : task.dueDate || taskStatusLabel(task)}</span></button>; })}</section></div></main>;
}
