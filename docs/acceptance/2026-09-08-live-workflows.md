...
- 补充失败边界：Core 回执已接受但首次快照重读失败时，命令层返回 accepted-pending，仍继续启动准备和最终 202 重读；任务删除后按 task ID 停止准备与待显示轮询。受管 output/material 目录逐级拒绝符号链接后才进入共享文件白名单。名单 ID 只对没有 `school-roster` 外部标识的旧实体启用直接 ID 兼容，显式冲突不会串到另一名学生。
- Desktop 全量 1124 tests 中 1099 passed、25 skipped、0 failed；TypeScript 与 ESLint 通过。该证据仍是源码隔离环境，不替代下一安装版、更多学科样本和真实课堂内容质量验收。
