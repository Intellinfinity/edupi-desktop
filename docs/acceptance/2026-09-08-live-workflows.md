...
## 2026-09-16 R14 公开包性能对比

- 使用已校验 SHA-256 的公开 `v0.3.13` 与 `v0.3.17` macOS updater 安全解包，在同一台 Apple Silicon Mac 上以独立空数据、独立 HOME/state 和无窗口 packaged server 交替启动，每版各 5 次。`v0.3.13` server ready / Core ready 暖缓存中位数为 240.6ms / 1114.1ms，`v0.3.17` 为 241.1ms / 1115.3ms；Core 中位数差 1.2ms（+0.1%）。两版首次读取分别为 5568.8ms 与 2476.8ms，受各包首次文件读取影响，只作为观测值，不据此声称代码提速。
- 每版在已就绪 server 上新建 3 个 1280×720 Chromium 上下文。`v0.3.13` / `v0.3.17` 页面首次可操作中位数为 864.3ms / 866.6ms，管理中心完成数据读取为 819.2ms / 819.7ms；系统切换 36.6ms / 37.2ms，自动运行切换 20.9ms / 17.9ms，材料切换 32.6ms / 33.4ms。六次页面运行的失败响应和 page error 均为 0。
- 结论限于公开包内置 server 和 Chromium 页面：`v0.3.17` 没有可复现的启动或模块切换回归，因此未增加推测性缓存或组件 memo。Tauri 原生窗口出现时间、拖动区、焦点返回、真实模型会话和 Windows 实机仍由安装验收覆盖。
