...
- 状态：macOS 安装版隔离数据验收大部分通过；成长方法“验证/发布”未完成，对话写回沿用开发证据，不能把 R20 整项勾完。
...
| 成长 | 新增“R20安装版对象连续性方法”，记录一次有效试用并关联已接受任务 | 通过创建与试用回读；点击“验证通过”超过 33 秒仍停留在“试用中”，未得到已验证/发布结果 |
...
- 成长方法“验证通过”无结果，需要单独复现网络/回执/组件状态，不能把方法已创建或已试用当成生命周期完成。
