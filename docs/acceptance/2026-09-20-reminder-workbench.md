...
- 使用相同隔离根另起当前源码 Next 服务 `127.0.0.1:30243`，只拦截该标签页 `/api/edupi/reminders` 的 Fetch/XHR 请求：响应暂停期间可访问树显示“正在读取提醒”且无错误 `0` 计数，放行后显示 `2 条待处理`；模拟读取失败时显示“提醒暂不可用”且不再显示空列表结论。800×900 实际页面 `scrollWidth=clientWidth=800`，正常路径控制台 error/warn 为 0。全量 `npm test` 为 1739 total / 1713 passed / 26 skipped / 0 failed；TypeScript、lint、`git diff --check` 通过。
