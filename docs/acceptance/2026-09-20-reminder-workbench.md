...
## 2026-09-25 共享通知临时放行后的送达核对

- macOS 通知设置原值为“当镜像或共享屏幕时：关闭通知”。经教师明确允许，临时切到“允许通知”并回读；已安装 v0.3.41 点击“测试通知跳转”后，系统日志 `/usr/bin/log show --last 5m --predicate 'process == "NotificationCenter" AND eventMessage CONTAINS[c] "com.abcwyc.pi-agent"' --info` 在 13:02:45 记录 `displaying as banner`、`addDelivered` 与 `Adding notification to storage`，DND `interruptionSuppression: none`。这证明系统层报告横幅展示并存储通知，超出“系统接受”，但没有屏幕捕获到横幅本身。
- Orca 与当前 Computer Use 可访问树只暴露通知中心小组件，没有可点的横幅/通知列表；尚未观察到点击后返回 EduPi 提醒。测试后立即将共享屏幕通知选项恢复为原值“关闭通知”，并在系统设置回读确认。通知点击需教师从菜单栏通知中心手动点击后再核对目标路由；不能用系统日志代替点击证据。

## 2026-09-25 macOS 安装版通知呈现条件（已由上节送达证据取代）
...
- 本段记录的是临时放行前的状态；上节实测后已恢复原有共享屏幕隐私设置。Apple [通知设置说明](https://support.apple.com/en-my/guide/mac-help/mchl205da693/mac) 说明共享/镜像时可以整体暂停通知。
