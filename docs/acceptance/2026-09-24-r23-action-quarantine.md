...
- 结论：风险门通过并已进入公开 v0.3.38；R23 仍为部分实现。
...
- 公开 v0.3.38 已消除旧生产入口；公开安装 smoke 只允许目录搜索/inspect，未配置或调用真实 Provider。
...
- 三平台签名构建与 Linux/Windows 公网安装已覆盖只读目录和执行阻断；旧环境变量下真实 Agent 工具表的安装版人工复核仍未单独操作。真实 Provider 一律未调用。
