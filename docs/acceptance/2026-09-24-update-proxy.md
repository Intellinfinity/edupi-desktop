...
状态：更新代理随 Desktop #251 与 Windows 覆盖修复 #255 进入公开签名 v0.3.39；macOS 安装版已完成原生保存、退出重启回读、Release 查询，以及经 7897 从 v0.3.39 下载、验签、覆盖安装并自动重启到 v0.3.40。Core 固定 `68004b2c0294159eef4f88bcbf4a921ef6978037`，本改动不修改教师数据。
...
| v0.3.39→v0.3.40 应用内原位升级 | 第一次到 2% 后下载中断且保留旧版；精确资产端点 range 请求经 7897 恢复后，第二次从 1%→99% 连续下载。PID `77509` 期间与 `127.0.0.1:7897` 保持 ESTABLISHED；页面进入“正在安装并重启”，旧 PID 退出、PID `85846` 以 v0.3.40 启动。升级后唯一安装副本签名/Gatekeeper、Core `68004b2`、51/240/43/9、五份配置摘要与代理 UI 回读均通过。 |

## 尚未验证

- macOS 的 7897 保存、重启回读、Release 查询、Tauri 清单/资产下载、验签、覆盖安装和自动重启均已通过。v0.3.38 及更早版本没有此设置，不能回溯证明旧包支持代理。Windows/Linux 原生设置的实际持久化和旧版应用内升级未实测；公开干净安装不能替代。现有 `ui-prefs.json` 其他写入路径的非原子性是历史问题，本新设置通过独立文件避免扩大其影响。
