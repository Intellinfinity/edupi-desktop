...
- 本机 macOS 系统 HTTP/HTTPS/SOCKS 代理仍指向 `127.0.0.1:7897`，但验收时该端口没有监听进程；因此未把真实 GitHub 经 7897 的下载、验签与重启记为通过。VPN 服务启动后需在签名安装版保存一次并执行旧版到新版更新，核对 Core、教师数据、模型配置与唯一安装副本。
