...
状态：更新代理功能已随 Desktop #251 合并为主线 `d2ef151`，Windows 覆盖替换修复正在独立收敛；尚未进入新的签名安装版。Core 仍固定 `68004b2c0294159eef4f88bcbf4a921ef6978037`，本改动不修改教师数据。
...
- 本机经 `127.0.0.1:7897` 访问 GitHub 已通过；该设置尚未进入当前 v0.3.38 安装包，因此未把真实应用内下载、验签与重启记为通过。装上首个支持该设置的签名版后，需保存一次代理，再升级到后续版本并核对 Core、教师数据、模型配置与唯一安装副本。
- 当前公开 v0.3.38 与已安装的 v0.3.37 都没有此设置；第一次获得支持该设置的签名版仍依赖旧更新链路可用，或需要一次经核验的安装引导。保存代理只对安装新版后的后续检查与升级生效，不宣称能回溯修好旧客户端。
- Windows/Linux 原生设置的实际持久化和包内更新链路未实测；Windows 仅完成源码编译和公开旧版安装。现有 `ui-prefs.json` 其他写入路径的非原子性是历史问题，本新设置通过独立文件避免扩大其影响。
