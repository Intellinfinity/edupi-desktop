...
状态：Desktop 实现提交 `4e2b7af` 已通过无签名预览；尚未合并、签名、安装或发布。Core 仍固定 `68004b2c0294159eef4f88bcbf4a921ef6978037`，本改动不修改教师数据。
...
- Windows/Linux 原生设置的持久化和包内更新链路未实测；本批只有无签名预览，没有正式签名安装包。现有 `ui-prefs.json` 其他写入路径的非原子性是历史问题，本新设置通过独立文件避免扩大其影响。
