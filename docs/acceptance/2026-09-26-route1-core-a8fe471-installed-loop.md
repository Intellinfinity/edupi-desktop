...
- 解锁后用最终源码另建独立 `com.abcwyc.pi-agent.route1-notify-canary`，复制至 `/tmp/edupi-route1-notify-final.ke49a7/Applications/`，只对该 bundle ID 打开 macOS 通知。包内原生 helper 逐层签名后，`.app` 的 `codesign --verify --deep --strict` 返回 `valid on disk` 和 `satisfies its Designated Requirement`；未做 Apple 公证。隔离工作区沿用上述测试根，模型只指向 `127.0.0.1:53008`。设置页回读“系统通知 已开启”，“测试通知跳转”返回“系统已接受通知”。
- 将签名 canary 关窗留后台，在同一隔离根创建 `teacher-task-79566176-aa2a-4e4d-a508-c38353dd4714` 当日事项。包内提醒 `03baa4dc-08e9-4bad-90e8-3fa455d90a40` 实际产生 `notificationAttemptedAt=2026-09-26T10:22:08.776Z`、`notificationDeliveredAt=2026-09-26T10:22:09.498Z`，失败次数为空；同时两条隔离提醒累计 native delivered 2。此处的 delivered 是原生 API 接受并持久记账，**不证明横幅可见或用户点击**：目前 `notificationOpenedAt=null`、点击数 0。macOS 屏幕共享时关闭通知的全局隐私选项未更改；等待教师点测试 canary 的系统通知核对同任务续聊。
- 真实跨到期睡眠另备隔离根 `/private/var/folders/xk/qmn_r8g93ljb7b5vqzq3rd040000gn/T/edupi-route1-packaged-loop-TvwbFM`：七一班数学周日与周一课表、已确认合成材料、仅本机 `127.0.0.1:53009` 的模型桩；独立 `EduPi Route1 Canary.app` 原生进程读取 Core `a8fe471`、G1 active、G2/G3 pending、`externalSend=false`。北京时间 9 月 26 日 18:51 前，周一 `teaching_before_class:timetable:route1-monday:2026-09-28` 的 work case `work_case_a323c943deb7aee8113d3b4b87b0b29e` 为 `planned`、产物 0；连续两次 `ensure` 与一次原生进程重启后仍为 planned、无模型调用。`pmset -g custom` 显示 AC/Battery 的自动 `sleep=0`，不能假设机器会自己睡；未改系统能源设置或设自动唤醒。只有跨 9 月 27 日 00:00 的**真实系统睡眠和唤醒**后，同一对象自动转为待审且仅调用模型一次，才可把本项记为通过；目前仍未验，等待教师手动睡眠和唤醒。
