...
| [ ] | R09 feat(editing): 教育对象统一编辑删除与历史 | 盘点并补齐教师资料、偏好、记忆、学生记录、教学重点、日程、材料元信息的手动编辑/AI 协作/删除/历史入口；AI 模板提供明确输入位置。 | 每类对象完成保存、取消、删除、历史查看及支持对象的恢复；重启保持；改几个字不强制开 AI；变更传入后续协作上下文。 | R08 | 部分实现；六类 Core 对象统一软删除、按需历史与安全恢复已通过，教师/校历/课表/材料操作历史和学生事件版本恢复可用；字段级历史、教学重点、学生资料与材料元信息版本仍待 Core 扩展 |
