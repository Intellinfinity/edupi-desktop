...
2. **R18 安装版交互。** 在 1458px 与 800px 原生窗口实际检查对话右上角“文件/已删除”、聊天历史滚动、抽屉和模块工具条；逐个用鼠标、Tab 与 Escape 操作。图标只用于明确的直接动作，保留可访问名称和悬浮说明；状态、权限、模型和不可逆决定继续写清文字。遇到重叠或无法点击即修复并复测，不能用开发版截图代替。
3. **R20 Core 对象写回。** 在隔离教师数据上从今天、任务、日历、材料和学生入口分别打开同一 Core 对象，实际编辑、审核、删除和恢复后从另一入口重新读取；返回原任务/会话并重启后再读一次。每类只在完整链路有证据时更新状态，现有深链回归不替代写回验收。
