...
- 本节原定 v0.3.32 继续固定 G6 `acce81e3…`；现已由上方独立 L4 配对更新为已合并 Core `860594a…`。新 pin 只来自干净 merge checkout，并已通过 packaged staged 验证；正式 v0.3.32 仍须在 Desktop PR 合并和远端质量门后重新构建，不能复用旧 G6 资产。R23 JEV 实服/受管浏览器/Core Receipt 与 OpenConnector 安装版 sidecar 仍未完成。
