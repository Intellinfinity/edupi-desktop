...
- v0.3.34 发布前 P2 已修：所有 review/task/memory/delete/restore mutation 都在验证回执后重读 occurrence v1.2 当前快照，教师资料审核后时间、时区、地点和 occurrence ref 不消失；旧 owner state 缺持久 key 时普通 Core 降级启动，owner 调用 fail closed，ambient 仍阻止启动，不自动重绑、不修改旧状态。全量 1424 passed / 26 skipped / 0 failed，source occurrence mutation E2 与真实旧状态升级演练通过。
...
- 版本边界：v0.3.33 已在 PR #212 合并前发布并通过 Linux/Windows 公共安装，只包含 Core G6；当前 occurrence/Core `860594a` 配对和两项 P2 修复由 v0.3.34 独立发布。不能把 main `1473bc5` 的合并状态写成 v0.3.33 已交付。
