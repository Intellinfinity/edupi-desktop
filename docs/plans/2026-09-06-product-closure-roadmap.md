...
## 2026-09-25 R18/R20/R21 AI 协作与提醒续聊复核（包内隔离验收，提醒加载修正待发布）
...
- 重现提醒工作台首次 GET 前短暂显示“0 条/暂无提醒”的误导状态；Desktop `4536d03` 改为仅在首次读取时显示“正在读取提醒”，稳定路由依赖避免反复加载。定向测试先失败后通过，全量 1712 passed / 26 skipped / 0 failed；当前源码隔离页面暂停 GET 时显示读取态、返回后显示 2 条，800×900 无横向溢出、控制台无错误。见 [提醒工作台验收](../acceptance/2026-09-20-reminder-workbench.md)。
- 该修正尚未进入公开 v0.3.40；macOS 原生系统通知点击、真实睡眠唤醒、正式模型内容和真实教师材料仍未因此验收，R21/L4 整体继续部分实现。
