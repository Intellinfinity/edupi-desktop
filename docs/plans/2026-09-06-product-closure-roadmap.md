...
- Core [#178](https://github.com/Intellinfinity/edupi/pull/178)–[#181](https://github.com/Intellinfinity/edupi/pull/181) 已依次合并，最终 merge `0f675489c5f1b8a35c5f32a0b74e6fc51f942758`。Core 现提供当前 canonical Goal/work-case/task 绑定、同 scope 单领域反馈目标、时钟/到期重查，以及 owner-message 撤回到 Goal、队列和 G2–G5 草稿的级联失效；ambient 停止时仍可完成 bounded withdrawal，但不能新 capture。Runtime manifest `sha256:c6cc01f7…`，schema 保持 `sha256:815f827e…`。
- Desktop 当前分支增加默认关闭的单班单科 G1 canary：7 天、最多 12 次模型调用、不外发。启停按数据根串行并使用 `updatedAt` CAS；停止不确定时保留 scope/grant 栅栏，只能重试停止。普通聊天仅在主 prompt 接受后、无正文 GET 证明 capability 和未过期 grant 后镜像；自然请求、修订、取消与重放由 Core 控制，多目标保持询问态。
...
- 工程证据：Core 全量、audit 与 `core-quality` run `35910317508` 通过；Desktop 1671 tests 中 1645 passed / 26 skipped / 0 failed，TypeScript、lint、actionlint、audit 通过。真实 merge Core E2 覆盖并发启用 CAS、请求/修订/取消、synthetic 反馈排除、重启、停止、会话删除撤回和 capture-crash pending 恢复；staged Desktop/feedback/occurrence/conflict/ICS/OCR/DOCX、Core closure 3/3、model host 2/2 通过，全部 `external_send=false`。完整记录见 [G1 主动运行试用验收](../acceptance/2026-09-24-proactivity-canary.md)。
