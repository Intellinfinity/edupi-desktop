...
- 该批完成六类 Core 对象在 Desktop 的统一软删除、按需历史、安全恢复和页面定位验收。教师资料字段级旧值、教学重点生命周期、学生资料版本和材料元信息版本还需要后续 Core 命令与存储，R09 保持部分实现。
...
| [ ] | R09 feat(editing): 教育对象统一编辑删除与历史 | 盘点并补齐教师资料、偏好、记忆、学生记录、教学重点、日程、材料元信息的手动编辑/AI 协作/删除/历史入口；AI 模板提供明确输入位置。 | 每类对象完成保存、取消、删除、历史查看及支持对象的恢复；重启保持；改几个字不强制开 AI；变更传入后续协作上下文。 | R08 | 部分实现；六类 Core 对象统一软删除、按需历史、安全恢复和页面定位已验收；教师/校历/课表/材料操作历史和学生事件版本恢复可用，字段级历史、教学重点、学生资料与材料元信息版本仍待 Core 扩展 |
