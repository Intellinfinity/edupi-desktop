...
- 主验收是隔离的单教师/单班/单科可信事件 → G1 后台到期草稿 → 通知点击续聊 → 教师审核及 Core 反馈。包内无 WebView 冷启曾红测无 Core DB，受身份校验的启动唤醒后 staged 实测 G1 active/G2/G3 pending；这不是安装版草稿/睡眠证据。G2/G3/G4 默认关闭且仅隔离 canary 可启用；G5 监护关系、材料和课次/学期归属未经 Core 证明时保持 hold，不自动外发。macOS/Windows 安装证据、PR/CI/合并状态分别记录，不提前打勾。
