...
## 2026-09-23 v0.3.32 公证 DMG 与 canonical 更新源（发布验收通过，安装升级待验收）
...
- R22 发布验收通过：首轮 `35771039553` 的 Linux/Windows 成功，macOS 因 G6 临时根 attestation 单点失败并保持 draft；同提交单平台 run `35772209888` 通过完整门禁。Release ID `394034871` 固定 merge `9f33463`，11 项资产、7 个签名 feed 键齐全。DMG submission `500735b4-4a03-4b0b-8376-c6cf3270c48c` 为 Accepted，runner staple/validate 与 Gatekeeper 通过，事务重传 SHA `3ee2667b…`；公开包本机 codesign 与 Gatekeeper 复核通过。完整证据见 [v0.3.32 公证 DMG 验收](../acceptance/2026-09-23-v0.3.32-notarized-dmg.md)。
...
- 本机唯一安装副本仍为 v0.3.29；代理环境中的更新接口已检测 v0.3.32，Core/投影/Kernel ready，51 学生/240 任务/43 校历/9 课表、模型/认证/设置摘要和手机默认关闭均已记录。当前 Mac 锁屏，原生更新点击、重启后版本/Core G6/数据与唯一副本核对仍待解锁，不能用公开包验收替代。
