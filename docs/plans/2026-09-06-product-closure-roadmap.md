...
- 独立 Desktop `codex/route1-core-a8fe471-20260926` 从 `9ff46e5` 起步；Core `main` 精确 `a8fe471`（#192 功能、#193 仅文档）。Runtime schema/bridge/课次 schema 不变，Runtime/ Desktop component manifest 已分别配对到 `844eebaf…` / `8092bd3d…`。隔离 macOS `.app` 已实测冷启 G1、单班数学 4 份内部草稿、失败通知站内回退、教师审核/Core synthetic 反馈、续聊绑定与原生重启保留。仍缺真实跨到期系统睡眠、系统通知成功点击和 Windows Core 原生根证明，故路线 1 保持“部分实现/部分验收”，不能记作安装版完整闭环。实施表见[路线 1 计划](2026-09-26-route1-core-a8fe471-installed-loop.md)，实际证据见[验收记录](../acceptance/2026-09-26-route1-core-a8fe471-installed-loop.md)。
