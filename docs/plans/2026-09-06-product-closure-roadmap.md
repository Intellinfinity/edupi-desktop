...
| [ ] | R03 fix(scheduler): 安装版自动调度与重启恢复 | 核查 Kernel 实际启动、触发器启用、时区、休眠唤醒和任务执行入口；将早安简报、课表和校历准备接到真实执行器；显示下次运行和失败原因。 | 安装版在指定时间触发一次；重启/唤醒按规则补跑，不重复产物；失败可重试；运行记录能对应具体任务和文件。 | R01 | 部分实现；v0.3.11 包内立即 due-scan 与重启恢复通过，Core 权威 Runtime/timer 状态和跨平台时钟间隔唤醒已合并，待下一安装版真实睡眠验收 |
