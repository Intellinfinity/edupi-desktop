...
- 桌面原生设置新增只接受无凭据本机 HTTP 地址的更新代理，独立持久化、损坏配置保守失败；Tauri updater 和设置/启动提醒的 Release 检查都使用同一代理，版本刷新有迟到结果栅栏。隔离本机 CONNECT、失败后缓存保持与重试、800×900 模拟桌面设置已通过；本机经 `127.0.0.1:7897` 请求 GitHub API 返回 200，但本功能尚未进入签名安装版，不能把命令行连通冒充应用内升级。证据见 [更新代理验收](../acceptance/2026-09-24-update-proxy.md)。R15/R22 整体仍部分验收，手机异地服务继续排最后。
