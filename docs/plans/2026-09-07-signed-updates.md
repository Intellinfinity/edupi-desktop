...
- Desktop `acd7ae4` 固定 Core `68004b2`，包含本机更新代理及 Windows 已有配置原子覆盖修复。[正式 run 35979258168](https://github.com/Intellinfinity/edupi-desktop/actions/runs/35979258168) 全绿；公开 [v0.3.39](https://github.com/Intellinfinity/edupi-desktop/releases/tag/v0.3.39) 为 Latest、11 项资产、7 个签名 updater 平台键。Apple 应用与 DMG 公证 Accepted，DMG staple/validate 和 Gatekeeper 通过；`updater-feed` 分支字节摘要与 Release `latest.json` digest 同为 `758e0d10…`，本机直连 Raw 超时。
- Linux [35984333853](https://github.com/Intellinfinity/edupi-desktop/actions/runs/35984333853)、Windows [35984333534](https://github.com/Intellinfinity/edupi-desktop/actions/runs/35984333534) 公网安装通过。本机 v0.3.37 经原生更新到 v0.3.39、自动重启，同路径 Core/投影/Kernel ready，51/240/43/9、模型/认证/设置和桌面偏好摘要保持，唯一安装副本签名有效；完整过程边界见 [v0.3.39 验收](../acceptance/2026-09-24-v0.3.39-signed-update-proxy.md)。
...
- 隔离代理 CONNECT、失败缓存/重试、800×900 页面、Rust/TypeScript 门禁、`35973703981` 无签名预览和 `35976790629` Windows 编译已有证据，见 [更新代理验收](../acceptance/2026-09-24-update-proxy.md)。v0.3.39 签名安装版的代理入口可见，默认留空使用系统网络；当前 7897 无监听，原生保存/重载与代理下载尚未验，不能用本次旧链路升级替代。
