...
## 2026-09-25 v0.3.41 发布准备（尚未发布）

- Desktop `4536d03` 与 `04ab688` 修正提醒请求前和读取失败时的错误空状态；隔离当前源码页面与全量门禁已通过，证据见 [提醒工作台验收](../acceptance/2026-09-20-reminder-workbench.md)。v0.3.41 版本元数据和说明已准备，`npm test` 1713 passed / 26 skipped / 0 failed、TypeScript、lint、audit（0 漏洞）、`release:verify`、Cargo locked metadata 与 30/30 原生测试通过。
- 公开 Latest 和本机安装版仍为 v0.3.40，均不含该修正。原生通知测试在 v0.3.40 显示“系统已接受通知”，但还未观察到可点击通知或返回提醒；真实睡眠唤醒与真实模型内容不因版本准备而视为通过。
