...
test("a cross-platform clock-gap monitor wakes one bounded Desktop Core catch-up", async () => {
  const [native, rust] = await Promise.all([
    source("lib/desktop-native.ts"),
    source("src-tauri/src/lib.rs"),
  ]);
  assert.match(native, /listen\("edupi:\/\/resume"/);
  assert.match(rust, /start_desktop_resume_monitor/);
  assert.match(rust, /RESUME_GAP_THRESHOLD/);
  assert.match(rust, /resume_gap_detected/);
  assert.match(rust, /emit\("edupi:\/\/resume", \(\)\)/);
  assert.doesNotMatch(rust, /RunEvent::Resumed/);
});
