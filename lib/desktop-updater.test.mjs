...
const { desktopUpgradeErrorMessage, downloadVerifiedUpdate } = await jiti.import("./desktop-updater.ts");
...
test("persistent network interruption returns the final failure without installing", async () => {
  ...
  assert.equal(downloads, 3);
  assert.equal(desktopUpgradeErrorMessage("error decoding response body"), "更新包下载中断，请检查网络后重试。");
  assert.equal(desktopUpgradeErrorMessage(new Error("Invalid signature")), "更新包验证失败，请稍后重试。");
});
