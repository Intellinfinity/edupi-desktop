...
test("a message outside the enabled teaching-preparation domain is tombstoned without resolution or application", async () => {
  const calls = [], ledger = [];
  const input = { ...base, messageId: "prompt-student", text: "请跟进学生张三的课堂观察" };
  const outRef = ambient.predictEduPiOwnerMessageRef(rootRef, ownerId, input.messageId);
  const host = {
    async call() { return { ok: true, result: { root_ref: rootRef, owner: { id: ownerId }, grants: [{ id: base.grantId, version: 2, status: "active", ends_at: "2026-09-30T00:00:00.000Z" }] } }; },
    async callOwnerControl(operation, payload) {
      calls.push([operation, payload]);
      if (operation === "owner_message" && payload.action === "capture") return { ok: true, result: { receipt: { message_ref: outRef, owner_id: ownerId, capture_grant_version: 2, external_send: false }, replayed: false } };
      if (operation === "owner_intent_read") return { ok: true, result: { status: "held", action: "abstain",
        reason: "domain_not_authorized", message_ref: outRef, external_send: false, candidate: null } };
      if (operation === "owner_message" && payload.action === "withdraw") return { ok: true, result: { replayed: false,
        receipt: { action: "withdraw", message_ref: outRef, revision: 2, owner_id: ownerId, root_ref: rootRef,
          recorded_at: "2026-09-23T08:00:01.000Z", apply: false, live_authority: false, external_send: false } } };
      throw new Error(`unexpected operation ${operation}`);
    },
  };
  const result = await ambient.captureAndApplyAmbientMessage(host, input, {
    controlScope: { classId: "class-7-1", subject: "数学" },
    onPrepared: async () => { ledger.push("pending"); },
    onCaptured: async () => { ledger.push("captured"); },
    onWithdrawn: async (binding) => { ledger.push(`withdrawn:${binding.withdrawnAt}`); },
  });
  assert.deepEqual(result, { status: "captured", resolutionStatus: "held", reason: "domain_out_of_scope",
    goalId: null, workCaseId: null, externalSend: false });
  assert.deepEqual(calls.map(([operation, payload]) => `${operation}:${payload.action || ""}`),
    ["owner_message:capture", "owner_intent_read:", "owner_message:withdraw"]);
  assert.deepEqual(ledger, ["pending", "captured", "withdrawn:2026-09-23T08:00:01.000Z"]);
});
