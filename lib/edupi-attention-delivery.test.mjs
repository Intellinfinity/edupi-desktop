...
const { selectNativeReminderNotifications, syncReminderAttention, revalidateG1LocalClaims } = await createJiti(import.meta.url).import("./edupi-attention-delivery.ts");
...
test("opening one exact reminder never writes Core opened evidence for a sibling on the same task", async () => {
  const sibling = { ...item, id: "reminder-2" };
  const deliveries = [];
  const host = { call: async (operation, payload) => {
    ...
    if (operation === "attention_delivery_record") deliveries.push(payload);
    return { ok: true, result: {} };
  } };
  await syncReminderAttention({ data, items: [item, sibling], action: { id: item.id, taskId: item.taskId, type: "notification_opened" },
    runtime: { host } });
  assert.deepEqual(deliveries.map(delivery => delivery.delivery_id), [item.id, item.id]);
  assert.deepEqual(deliveries.map(delivery => delivery.status), ["queued", "opened"]);
});

test("a persisted Core carrier survives Desktop restart for claim and outcome replay", async () => {
  const instanceId = "previous-desktop-instance";
  const calls = [];
  const host = { call: async (operation, payload) => {
    calls.push([operation, payload]);
    if (operation === "health") return { ok: true, result: { data_root_fingerprint: `sha256:${"a".repeat(64)}`,
      capabilities: { supported_operations: ["attention_delivery_record", "attention_delivery_read"] } } };
    if (operation === "attention_delivery_read") return { ok: true, result: { root_ref: `sha256:${"a".repeat(64)}`,
      apply: false, external_send: false, deliveries: [{ delivery_id: item.id,
        attention_intent_id: intent.attentionIntentId, opportunity_id: intent.opportunityId,
        work_case_id: intent.workCaseId, deep_link: intent.deepLink, carrier: { kind: "desktop", instance_id: instanceId },
        status: "queued", intent_current: true, external_send: false }] } };
    return { ok: true, result: {} };
  } };
  const result = await syncReminderAttention({ data, items: [item], action: { id: "*", type: "claim_notifications" },
    instanceId, runtime: { host } });
  assert.deepEqual(result.currentNotificationIds, [item.id]);
  assert.equal(calls.find(([operation]) => operation === "attention_delivery_record")[1].carrier.instance_id, instanceId);
  assert.equal(calls.find(([operation]) => operation === "attention_delivery_read")[1].carrier.instance_id, instanceId);
  const invalid = await syncReminderAttention({ data, items: [item], action: { id: "*", type: "claim_notifications" },
    instanceId: "invalid:carrier", runtime: { host: { call: async () => { throw new Error("should not call Core"); } } } });
...
test("a current G1-only completion needs explicit Core G1/local-only proof and never overrides a linked intent", async () => {
  const g1 = { ...item, id: "g1-completion", kind: "ready", nativeSource: "core_g1" };
  const host = { call: async operation => operation === "health"
    ? { ok: true, result: { data_root_fingerprint: `sha256:${"a".repeat(64)}`, capabilities: {
      supported_operations: ["attention_delivery_record", "attention_delivery_read"],
      g1_processor: "active", ambient_planning: "activation_pending", attention_delivery: "activation_pending",
    } } }
    : { ok: false, error_code: "activation_pending" } };
  const localOnly = await syncReminderAttention({ data: { ...data, l4Preparation: null }, items: [g1],
    action: { id: "*", type: "claim_notifications" }, runtime: { host } });
  assert.equal(localOnly.g1LocalFallback, true);
  assert.deepEqual(selectNativeReminderNotifications([g1], localOnly).map(candidate => candidate.id), [g1.id]);
  assert.deepEqual(selectNativeReminderNotifications([g1], { ...localOnly, linkedNotificationIds: [g1.id] }), []);
  const unavailable = await syncReminderAttention({ data: { ...data, l4Preparation: null }, items: [g1],
    action: { id: "*", type: "claim_notifications" }, runtime: { host: { call: async () => { throw new Error("offline"); } } } });
  assert.deepEqual(selectNativeReminderNotifications([g1], unavailable), []);
});

test("G1 local send rechecks the current task, work case, artifacts and L4 mode before authorization", async () => {
  const task = { id: "teaching_before_class:timetable:slot-1:2026-09-27", title: "数学备课", trigger: "teaching_before_class",
    sourceEventId: "timetable:slot-1:2026-09-27", status: "planned", contentStatus: "draft_ready",
    scope: "teacher_internal", requiresTeacherReview: true, externalSend: false, evidence: {} };
  const workCase = { id: "work_case_1", taskId: task.id, kind: "teaching_before_class", currentState: "draft_ready",
    artifactIds: ["artifact-1"], externalSend: false };
  const artifact = { artifact_id: "artifact-1", task_id: task.id, available: true };
  const current = { scope: "teacher_internal", externalSend: false, requiresTeacherReview: true, workspace: "/tmp/route1",
    tasks: [task], workCases: [workCase], generatedArtifacts: [artifact], continuity: { documents: [] }, l4Preparation: null };
  const event = reminderEvents(current.tasks, current.workspace, new Date(), [], current.workCases, current.generatedArtifacts)[task.id];
  const g1 = { id: "r-g1", taskId: task.id, title: task.title, kind: "ready", identity: event.identity, nativeSource: "core_g1" };
  const manual = { id: "r-manual", taskId: "manual-task", title: "手动事项", kind: "due", identity: "due:today", nativeSource: "teacher_created" };
  const claims = [g1, manual];
  assert.deepEqual((await revalidateG1LocalClaims(claims, async () => current)).map(item => item.id), [g1.id, manual.id]);
  for (const stale of [
    { ...current, tasks: [{ ...task, status: "accepted" }] },
    { ...current, workCases: [{ ...workCase, currentState: "failed" }] },
    { ...current, generatedArtifacts: [{ ...artifact, available: false }] },
    { ...current, l4Preparation: { attentionIntents: [], attentionDeliveries: [] } },
  ]) {
    assert.deepEqual((await revalidateG1LocalClaims(claims, async () => stale)).map(item => item.id), [manual.id]);
  }
  assert.deepEqual((await revalidateG1LocalClaims(claims, async () => { throw new Error("Core unavailable"); })).map(item => item.id), [manual.id]);
});
