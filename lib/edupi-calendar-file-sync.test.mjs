...
test("an ICS upload must explicitly replace a matching document source instead of duplicating it", async () => {
  const sourceId = "document-source-00000000000000000000000000000014";
  const prior = event("document-occurrence-teacher-meeting", "2026-10-01", "教研会");
  const incoming = event("ics-uid-teacher-meeting", "2026-10-02", "教研会");
  const before = coreSources(sourceId, [prior]);
  await assert.rejects(sync.syncCalendarFile({ descriptor, requestedSourceId: null, expectedSourceFingerprint: null }, {
    recognize: async () => ({ events: [incoming], slots: [], calendar_mode: "full_snapshot" }),
    readSources: async () => sourceRead(before), readDeletions: async () => ledger(), issue: acceptedIssue([]),
  }), (error) => error?.code === "calendar_source_selection_required");

  const afterImport = coreSources(sourceId, [prior, incoming], [evidenceId]);
  const deleted = [];
  const commands = [];
  const result = await sync.syncCalendarFile({ descriptor, requestedSourceId: sourceId, expectedSourceFingerprint: before[0].fingerprint }, {
    recognize: async () => ({ events: [incoming], slots: [], calendar_mode: "full_snapshot" }),
    readSources: async () => sourceRead(before),
    readSourcesAfter: async () => sourceRead(afterImport),
    readDeletions: async () => ledger(),
    issue: acceptedIssue(commands),
    deleteCalendarBatch: async (eventIds) => { deleted.push(...eventIds); },
  ...
  assert.equal(result.committed, true);
  assert.equal(commands.at(-1).source.source_id, sourceId);
  assert.deepEqual(deleted, [upload.stableOccurrenceCalendarEventId(sourceId, prior.source_occurrence_ref)]);
});
