...
test("brief calendar links resolve only the referenced object on its date", async () => {
  const { calendarSelectionForSource, calendarSelectionFromLink, calendarSelectionLink } = await jiti.import("./edupi-calendar-model.ts");
  ...
  assert.deepEqual(calendarSelectionForSource(data, "calendar", "event:1"), { kind: "calendar", sourceId: "event:1", date: "2026-09-09", title: "教研", detail: undefined, sourceLabel: "校历", statusLabel: "待确认" });
  assert.deepEqual(calendarSelectionForSource(data, "timetable", "slot:1"), { kind: "timetable", sourceId: "slot:1", date: null, title: "数学", detail: "周3 · 第 2 节 · 703", sourceLabel: "课程表", statusLabel: "已确认" });
  assert.equal(calendarSelectionForSource(data, "calendar", "missing"), null);
  assert.deepEqual(calendarSelectionLink(calendarSelectionForSource(data, "calendar", "event:1")), { kind: "calendar", id: "event:1", date: "2026-09-09" });
  assert.deepEqual(calendarSelectionLink(calendarSelectionForSource(data, "timetable", "slot:1")), { kind: "timetable", id: "slot:1", date: null });
  assert.equal(calendarSelectionLink({ kind: "calendar", sourceId: null, date: "2026-09-09", title: "未保存", detail: null, sourceLabel: "教师", statusLabel: "待确认" }), null);
  assert.equal(calendarSelectionFromLink(data, { kind: "timetable", id: "slot:1", date: null }).sourceId, "slot:1");
});
const {
  addCalendarMonths,
