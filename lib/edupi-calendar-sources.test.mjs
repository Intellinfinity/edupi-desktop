...
test("rebuilds calendar source identity, current occurrences, and CAS fingerprint only from Core", () => {
  ...
  assert.equal(projected[0].sourceKind, "calendar");
...
test("projects document schedule sources separately from uploaded ICS sources", () => {
  const calendarSource = "calendar-source-00000000000000000000000000000001";
  const documentSource = "document-source-00000000000000000000000000000002";
  const projected = sources.projectCoreCalendarSources([
    event(calendarSource, "uid-1"),
    { ...event(documentSource, "document-occurrence-1", "家长会", "2026-10-03"), confidence: "inferred" },
  ]);
  assert.deepEqual(projected.map((item) => [item.sourceId, item.sourceKind]), [
    [calendarSource, "calendar"],
    [documentSource, "document"],
  ]);
  assert.deepEqual(sources.calendarSourceSelectionCandidates(projected,
    [event("ignored", "document-occurrence-1")], [], "document").map((item) => item.sourceId), [documentSource]);
  assert.deepEqual(sources.calendarSourceSelectionCandidates(projected,
    [event("ignored", "document-occurrence-1")]).map((item) => item.sourceId), [calendarSource]);
});

test("projects a byte-revision alias only from unique Core schedule evidence", () => {
  const documentSource = "document-source-00000000000000000000000000000002";
  const calendarSource = "calendar-source-00000000000000000000000000000003";
  const derived = `document-source-${"b".repeat(32)}`;
  const evidenceId = `schedule-evidence-${"b".repeat(32)}`;
  const projected = sources.projectCoreCalendarSources([
    { ...event(documentSource, "document-ref"), evidence_ids: [evidenceId] },
    { ...event(calendarSource, "calendar-ref", "另一个事项"), evidence_ids: ["schedule-evidence-other"] },
  ]);
  assert.deepEqual(sources.documentSourceEvidenceAliasCandidates(projected, derived).map((source) => source.sourceId), [documentSource]);
  assert.deepEqual(sources.documentSourceEvidenceAliasCandidates(projected, "calendar-source-not-document"), []);
  const ambiguous = sources.projectCoreCalendarSources([
    { ...event(documentSource, "document-ref"), evidence_ids: [evidenceId] },
    { ...event(calendarSource, "calendar-ref", "另一个事项"), evidence_ids: [evidenceId] },
  ]);
  assert.equal(sources.documentSourceEvidenceAliasCandidates(ambiguous, derived).length, 2);
});
...
test("fails closed on ambiguous source ownership or duplicate Core occurrences", () => {
  ...
  assert.throws(() => sources.projectCoreCalendarSources([{ ...event(first, "uid-bad"), source_ids: ["calendar-source-not-valid"] }]),
    (error) => error?.code === "invalid_calendar_source_projection");
  assert.throws(() => sources.projectCoreCalendarSources([{ ...event(first, "uid-bad"), source_ids: ["document-source-not-valid"] }]),
    (error) => error?.code === "invalid_calendar_source_projection");
