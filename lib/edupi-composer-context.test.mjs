...
const { contextHandoffMode, createComposerContext, composeTeacherMessage, isGeneratedContextDraft, parseTeacherMessage, prepareQueueRecall, visibleTeacherMessageText } = await createJiti(import.meta.url).import("./edupi-composer-context.ts");
...
test("recalling different queued contexts keeps every reference readable without a hidden wrapper", () => {
  ...
  assert.equal(recalled.context, null);
  assert.match(recalled.text, /事项：单元练习[\s\S]*检查题目/);
  assert.match(recalled.text, /事项：李四的档案[\s\S]*核对备注/);
  assert.match(recalled.text, /老师要求：原有草稿/);
  ...
  assert.equal(contextOnly.context, null);
  assert.match(contextOnly.text, /事项：单元练习/);
  assert.match(contextOnly.text, /事项：李四的档案/);
});
