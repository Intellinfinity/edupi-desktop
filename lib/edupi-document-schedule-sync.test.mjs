...
const timetableAlias = await jiti.import("./edupi-timetable-source-alias.ts");
...
test("replays a slot-only document under its proven Core source and rejects stale slot evidence", async () => {
  const sourceId = "document-source-00000000000000000000000000000041";
  const evidenceId = `schedule-evidence-${descriptor.source_hash.slice(7, 39)}`;
  const slot = { slot_id: "model-slot", day_of_week: 1, period: 2, subject: "数学",
    class_name: "七年级二班", kind: "class", notes: null };
  const row = { ...slot, slot_id: "canonical-slot", notes: "材料识别待确认：",
    source_ids: [sourceId], evidence_ids: [evidenceId] };
  const targets = [{ projection_kind: "material_intake", source_hash: descriptor.source_hash,
    status: "accepted", intake_state: "accepted", target: { target_kind: "material_intake", target_id: "material-target" } },
  { projection_kind: "timetable_import", source_hash: descriptor.source_hash, status: "accepted",
    source_ids: [sourceId], evidence_ids: [evidenceId], item_count: 1, conflict_count: 0, held_count: 0 }];
  const read = (timetable, reviewTargets = targets) => ({ sources: [], snapshot: {
    payload: { snapshot_id: "slot-snapshot", education_workspace: { timetable }, review_targets: reviewTargets },
    roots: { runtime: {}, dataRoot: {} }, occurrenceEvents: [],
  } });
  const input = { descriptor, title: "课表", materialKind: "other", subject: "数学", classId: "七年级二班",
    requestedSourceId: null, expectedSourceFingerprint: null };
  const commands = [];
  const replay = await sync.syncDocumentScheduleFile(input, {
    recognize: async () => ({ events: [], slots: [slot] }), readSources: async () => read([row]), issue: acceptedIssue(commands),
  });
  assert.equal(replay.committed, true);
  assert.equal(replay.sourceId, sourceId);
  assert.equal(commands.find((command) => command.command_type === "import_timetable").source.source_id, sourceId);
  const selectedFingerprint = timetableAlias.projectTimetableSourceOptions(read([row]))[0].fingerprint;
  const selectedCommands = [];
  const selected = await sync.syncDocumentScheduleFile({ ...input, requestedSourceId: sourceId,
    expectedSourceFingerprint: selectedFingerprint }, {
    recognize: async () => ({ events: [], slots: [slot] }), readSources: async () => read([row]),
    issue: acceptedIssue(selectedCommands),
  });
  assert.equal(selected.sourceId, sourceId);
  assert.equal(selectedCommands.find((command) => command.command_type === "import_timetable").source.source_id, sourceId);
  await assert.rejects(sync.syncDocumentScheduleFile({ ...input, requestedSourceId: sourceId,
    expectedSourceFingerprint: `sha256:${"f".repeat(64)}` }, {
    recognize: async () => ({ events: [], slots: [slot] }), readSources: async () => read([row]),
    issue: async () => { throw new Error("stale selection must not write"); },
  }), (error) => error?.code === "stale_calendar_source");
  await assert.rejects(sync.syncDocumentScheduleFile(input, {
    recognize: async () => ({ events: [], slots: [slot] }), readSources: async () => read([]),
    issue: async () => { throw new Error("stale slot must not write"); },
  }), (error) => error?.code === "calendar_source_selection_required");
  const freshCommands = [];
  const fresh = await sync.syncDocumentScheduleFile(input, {
    recognize: async () => ({ events: [], slots: [slot] }), readSources: async () => read([], []),
    issue: acceptedIssue(freshCommands),
  });
  assert.equal(fresh.committed, true);
  assert.equal(fresh.sourceId, upload.stableDocumentScheduleSourceId(descriptor.source_hash));
  assert.equal(freshCommands.find((command) => command.command_type === "import_timetable").source.source_id,
    upload.stableDocumentScheduleSourceId(descriptor.source_hash));
});

test("replays a mixed document only when calendar and timetable prove the same source", async () => {
  const sourceId = "document-source-00000000000000000000000000000042";
  const event = recognizedEvent();
  const evidenceId = `schedule-evidence-${descriptor.source_hash.slice(7, 39)}`;
  const current = coreSource(sourceId, [{ ...event,
    source_occurrence_ref: upload.stableDocumentOccurrenceRef(event), evidence_ids: [evidenceId] }]);
  const slot = { slot_id: "model-slot", day_of_week: 1, period: 2, subject: "数学",
    class_name: "七年级二班", kind: "class", notes: null };
  const row = { ...slot, slot_id: "canonical-slot", notes: "材料识别待确认：",
    source_ids: [sourceId], evidence_ids: [evidenceId] };
  const read = { sources: [current], snapshot: { payload: { snapshot_id: "mixed-snapshot",
    education_workspace: { timetable: [row] }, review_targets: [
      { projection_kind: "material_intake", source_hash: descriptor.source_hash, status: "accepted", intake_state: "accepted",
        target: { target_kind: "material_intake", target_id: "mixed-material" } },
      { projection_kind: "timetable_import", source_hash: descriptor.source_hash, status: "accepted", source_ids: [sourceId],
        evidence_ids: [evidenceId], item_count: 1, conflict_count: 0, held_count: 0 },
    ] }, roots: { runtime: {}, dataRoot: {} }, occurrenceEvents: [] } };
  const commands = [];
  const result = await sync.syncDocumentScheduleFile({ descriptor, title: "混合材料", materialKind: "other", subject: "数学",
    classId: "七年级二班", requestedSourceId: null, expectedSourceFingerprint: null }, {
    recognize: async () => ({ events: [event], slots: [slot] }), readSources: async () => read,
    issue: acceptedIssue(commands),
  });
  assert.equal(result.committed, true);
  assert.equal(result.sourceId, sourceId);
  assert.deepEqual(commands.filter((command) => command.command_type.startsWith("import_")).map((command) => command.source.source_id),
    [sourceId, sourceId]);
  const editedRead = { ...read, snapshot: { ...read.snapshot, payload: { ...read.snapshot.payload,
    education_workspace: { timetable: [{ ...row, notes: "教师已更正" }] },
  } } };
  const explicitCommands = [];
  const explicit = await sync.syncDocumentScheduleFile({ descriptor, title: "混合材料", materialKind: "other", subject: "数学",
    classId: "七年级二班", requestedSourceId: sourceId, expectedSourceFingerprint: current.fingerprint }, {
    recognize: async () => ({ events: [event], slots: [slot] }), readSources: async () => editedRead,
    issue: acceptedIssue(explicitCommands),
  });
  assert.equal(explicit.sourceId, sourceId, "explicit calendar source bypasses automatic slot alias recovery");
});

test("a legacy slot-only source cannot be used for newly recognized calendar events", async () => {
  const sourceId = `desktop-file-schedule-${"d".repeat(24)}`;
  const evidenceId = `schedule-evidence-${descriptor.source_hash.slice(7, 39)}`;
  const slot = { slot_id: "model-slot", day_of_week: 1, period: 2, subject: "数学",
    class_name: "七年级二班", kind: "class", notes: null };
  const read = { sources: [], snapshot: { payload: { snapshot_id: "legacy-slot-snapshot",
    education_workspace: { timetable: [{ ...slot, notes: "材料识别待确认：",
      source_ids: [sourceId], evidence_ids: [evidenceId] }] }, review_targets: [
      { projection_kind: "material_intake", source_hash: descriptor.source_hash, status: "accepted",
        intake_state: "accepted", target: { target_kind: "material_intake", target_id: "material-legacy" } },
      { projection_kind: "timetable_import", source_hash: descriptor.source_hash, status: "accepted",
        source_ids: [sourceId], evidence_ids: [evidenceId], item_count: 1, conflict_count: 0, held_count: 0 },
    ] }, roots: { runtime: {}, dataRoot: {} }, occurrenceEvents: [] } };
  await assert.rejects(sync.syncDocumentScheduleFile({ descriptor, title: "旧课表新增事项", materialKind: "other",
    subject: "数学", classId: "七年级二班", requestedSourceId: null, expectedSourceFingerprint: null }, {
    recognize: async () => ({ events: [recognizedEvent()], slots: [slot] }), readSources: async () => read,
    issue: async () => { throw new Error("legacy calendar must not write"); },
  }), (error) => error?.code === "calendar_source_selection_required");
});
