...
test("classifies insights and materials into one stable category", () => {
  assert.deepEqual(navigation.INSIGHT_STATUSES.map((item) => item.label), ["全部", "原始观察", "已确认事实", "待确认事实", "已暂缓事实", "已浮出", "酝酿中", "弱信号"]);
  ...
  assert.equal(navigation.materialCategory({ materialKind: "worksheet", title: "移项专项教案" }), "practice");
  assert.equal(navigation.materialCategory({ materialKind: "other", title: "课堂观察记录" }), "other");
...
test("review routes preserve the exact Core target identity", () => {
  const observation = navigation.reviewTargetObjectId({ kind: "observation", id: "observation:a/b" });
  assert.equal(observation, "observation:observation%3Aa%2Fb");
  assert.deepEqual(navigation.reviewTargetRoute(observation), { kind: "observation", id: "observation:a/b" });
  const memory = navigation.reviewTargetObjectId({ kind: "memory_candidate", id: "candidate:1" });
  assert.deepEqual(navigation.reviewTargetRoute(memory), { kind: "memory_candidate", id: "candidate:1" });
  assert.equal(navigation.reviewTargetRoute("task:1"), null);
  assert.equal(navigation.reviewTargetRoute("observation:%E0%A4%A"), null);
});
