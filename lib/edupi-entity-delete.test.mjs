...
test("entity deletion request is bounded to the seven Core-owned target kinds", () => {
  assert.deepEqual(ENTITY_DELETE_KINDS, ["calendar", "timetable", "memory", "student", "task", "material", "teaching_priority"]);
...
const restoreInput = (record = deletionRecord, id = record.id) => ({ kind: record.kind, id, note: null, restoreRequestId: entityRestoreRequestId(record) });
...
test("deletion ledger accepts only bounded Core-owned tombstones and history", async () => {
  ...
  assert.equal(ledger.historyTruncated, false);
  const truncated = await readEntityDeletionLedger({}, { roots, callCore: async (request) => ({
    ok: true, operation: "delete", action: "list", request_id: request.request_id, snapshot_id: "snapshot-truncated",
    external_send: false, history_truncated: true, deletions: [], history: [],
  }) });
...
test("entity restore re-reads the Core ledger and requires the object in the refreshed projection", async () => {
  ...
  const result = await issueEntityRestore(restoreInput(), {
    roots,
  ...
  await assert.rejects(() => issueEntityRestore(restoreInput(), {
    roots,
    readLedger: async () => ({ snapshotId: "snapshot-deleted", deletions: [deletionRecord], history: [] }),
...
test("entity restore prefers an exact tombstone ID and rejects ambiguous legacy aliases", async () => {
  const roots = { runtime: {}, dataRoot: {} };
  const aliasOwner = { ...deletionRecord, id: "student-a", studentId: "student-b", targetFingerprint: `sha256:${"b".repeat(64)}`, tombstoneRevision: 3 };
  const exactTarget = { ...deletionRecord, id: "student-b", studentId: null, targetFingerprint: `sha256:${"c".repeat(64)}`, tombstoneRevision: 4 };
  let selectedRequest = null;
  await issueEntityRestore(restoreInput(exactTarget), {
    roots,
    readLedger: async () => ({ snapshotId: "snapshot-collision", deletions: [aliasOwner, exactTarget], history: [] }),
    callCore: async (request) => {
      selectedRequest = request;
      return { ok: true, operation: "delete", action: "restore", request_id: request.request_id, target: { kind: "student", id: "student-b" }, tombstone_revision: exactTarget.tombstoneRevision, target_fingerprint: exactTarget.targetFingerprint, external_send: false, snapshot: { education_workspace: { ...workspace, students: [{ student_id: "student-b", name: "王五" }] } } };
    },
  });
  assert.equal(selectedRequest.expected_tombstone_revision, exactTarget.tombstoneRevision);
  assert.equal(selectedRequest.expected_target_fingerprint, exactTarget.targetFingerprint);
  await assert.rejects(() => issueEntityRestore({ ...restoreInput(exactTarget), restoreRequestId: entityRestoreRequestId(aliasOwner) }, {
    roots,
    readLedger: async () => ({ snapshotId: "snapshot-newer-delete", deletions: [aliasOwner, exactTarget], history: [] }),
    callCore: async () => { throw new Error("a stale restore generation must fail before Core"); },
  }), (error) => error instanceof EntityDeleteError && error.code === "stale_tombstone");

  const secondAlias = { ...deletionRecord, id: "student-c", studentId: "legacy-student", targetFingerprint: `sha256:${"d".repeat(64)}`, tombstoneRevision: 5 };
  await assert.rejects(() => issueEntityRestore(restoreInput(aliasOwner, "legacy-student"), {
    roots,
    readLedger: async () => ({ snapshotId: "snapshot-ambiguous-alias", deletions: [{ ...aliasOwner, studentId: "legacy-student" }, secondAlias], history: [] }),
    callCore: async () => { throw new Error("ambiguous aliases must fail before Core"); },
  }), (error) => error instanceof EntityDeleteError && error.code === "legacy_identity_unresolvable");
});

test("entity restore reconciles a committed Core history after its response times out", async () => {
  const roots = { runtime: {}, dataRoot: {} };
  let ledgerReads = 0;
  let committedHistory = null;
  const legacyAliasRecord = { ...deletionRecord, studentId: "legacy-student" };
  const unrelatedAliasTombstone = { ...deletionRecord, id: "student-a", studentId: "student-1", targetFingerprint: `sha256:${"e".repeat(64)}`, tombstoneRevision: 6 };
  const result = await issueEntityRestore(restoreInput(legacyAliasRecord, "legacy-student"), {
    roots,
    readLedger: async () => {
      ledgerReads += 1;
      return committedHistory
        ? { snapshotId: "snapshot-restored", deletions: [unrelatedAliasTombstone], history: [committedHistory] }
        : { snapshotId: "snapshot-deleted", deletions: [legacyAliasRecord], history: [] };
    },
    callCore: async (request) => {
      committedHistory = {
        historyId: "history-timeout",
        requestId: request.request_id,
        action: "restore",
        kind: "student",
        targetId: "student-1",
        targetLabel: "李四",
        targetFingerprint: legacyAliasRecord.targetFingerprint,
        tombstoneRevision: legacyAliasRecord.tombstoneRevision,
        beforeSnapshotId: "snapshot-deleted",
        afterSnapshotId: "snapshot-restored",
    ...
    readCurrentSnapshot: async () => ({ education_workspace: { ...workspace, students: [{ student_id: "student-1", name: "李四" }] } }),
  });
  assert.equal(ledgerReads, 2);
  assert.equal(result.target.id, "student-1");
  assert.equal(result.restoredAt, "2026-09-14T03:00:00.000Z");
});

test("entity restore treats a matching Core request history as an idempotent retry", async () => {
  const roots = { runtime: {}, dataRoot: {} };
  const legacyAliasRecord = { ...deletionRecord, studentId: "legacy-student" };
  const priorRestore = {
    historyId: "history-prior-restore",
    requestId: entityRestoreRequestId(deletionRecord),
    action: "restore",
    kind: "student",
    targetId: "student-1",
    targetLabel: "李四",
    targetFingerprint: deletionRecord.targetFingerprint,
    tombstoneRevision: deletionRecord.tombstoneRevision,
    beforeSnapshotId: "snapshot-deleted",
    afterSnapshotId: "snapshot-restored",
    occurredAt: "2026-09-14T03:05:00.000Z",
    reviewer: "teacher",
    note: null,
  };
  const result = await issueEntityRestore(restoreInput(legacyAliasRecord, "legacy-student"), {
    roots,
    readLedger: async () => ({ snapshotId: "snapshot-restored", deletions: [{ ...deletionRecord, id: "student-a", studentId: "student-1", targetFingerprint: `sha256:${"f".repeat(64)}`, tombstoneRevision: 7 }], history: [priorRestore] }),
    readCurrentSnapshot: async () => ({ education_workspace: { ...workspace, students: [{ student_id: "student-1", name: "李四" }] } }),
    callCore: async () => { throw new Error("an already reconciled retry must not call Core mutation"); },
  });
  assert.equal(result.target.id, "student-1");
  assert.equal(result.restoredAt, priorRestore.occurredAt);
});
