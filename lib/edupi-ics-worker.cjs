...
function seriesReference(uid) {
  return `ics-series:${crypto.createHash("sha256").update(uid, "utf8").digest("hex")}`;
}

function normalizedReference(uid, recurrenceKey = null) {
  if (recurrenceKey) return `${seriesReference(uid)}#${recurrenceKey}`;
  const candidate = uid;
  if (candidate.startsWith("ics-series:") || candidate.startsWith("ics:")) {
...
async function main() {
  ...
  const explicitCancelledRefs = new Set();
  const cancellationOnlyUids = new Set();
  const affectedSeriesRefs = new Set();
  for (const event of parsedSource.events) {
    ...
    const cancelledEvent = parsedSource.calendarMethod === "CANCEL" || status?.value.toUpperCase() === "CANCELLED";
    const recurrenceValue = recurrence ? parseDateTime(recurrence, Temporal) : null;
    if (cancelledEvent && recurrenceValue) {
      if (recurrenceValue.dateOnly) fail("ambiguous_schedule", "Recurring all-day ICS cancellations are unsupported.");
      explicitCancelledRefs.add(normalizedReference(uid, new Date(recurrenceValue.zoned.epochMilliseconds).toISOString()));
      cancellationOnlyUids.add(uid);
      continue;
    }
    const startProperty = one(event, "DTSTART");
    const ruleProperty = one(event, "RRULE");
    if (cancelledEvent && !recurrence) {
      affectedSeriesRefs.add(seriesReference(uid));
      cancellationOnlyUids.add(uid);
      continue;
    }
    const start = parseDateTime(startProperty, Temporal);
    ...
    if (!cancelledEvent && !recurrence && ["REQUEST", "PUBLISH"].includes(parsedSource.calendarMethod)) {
      affectedSeriesRefs.add(seriesReference(uid));
    }
  ...
  const events = [];
  const cancelled = new Set(explicitCancelledRefs);
  for (const component of Object.values(response)) {
    if (!component || component.type !== "VEVENT") continue;
    const uid = component.uid;
    const meta = metadata.get(uid);
    if (!meta) {
      if (cancellationOnlyUids.has(uid)) continue;
      fail("invalid_output", "ICS parser identity mismatch.");
    }
  ...
  if (events.length + cancelled.size > MAX_EVENTS || affectedSeriesRefs.size > MAX_EVENTS) fail("too_large", "ICS contains too many occurrences.");
  ...
  const calendarMode = parsedSource.calendarMethod === "CANCEL" || events.length === 0 && (cancelled.size > 0 || affectedSeriesRefs.size > 0)
    ? "delta_cancel"
  ...
  return { events, slots: [], calendar_mode: calendarMode,
    ...(cancelled.size ? { cancelled_occurrence_refs: [...cancelled].sort() } : {}),
    ...(affectedSeriesRefs.size ? { affected_series_refs: [...affectedSeriesRefs].sort() } : {}) };
}
