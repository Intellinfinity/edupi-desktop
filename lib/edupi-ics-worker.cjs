...
async function main() {
  ...
  for (const event of parsedSource.events) {
    ...
    if (cancelledEvent && !startProperty) {
      if (ruleProperty) fail("ambiguous_schedule", "Cancelling a recurrence series requires its bounded start.");
      explicitCancelledRefs.add(normalizedReference(uid));
      cancellationOnlyUids.add(uid);
      continue;
    }
