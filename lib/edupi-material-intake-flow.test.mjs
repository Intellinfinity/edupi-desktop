...
test("exact file bytes keep one schedule identity after a rename without sharing material provenance", async () => {
  const commands = [];
...
test("source-verified typed times distinguish same-day events without collapsing them", async () => {
  const commands = [];
  const event = (hour) => ({ event_id: `typed-${hour}`, date: "2026-10-20", end_date: null, name: "教研会", type: "meeting",
    confidence: "inferred", notes: null,
    time_interval: { start: `2026-10-20T${hour}:00+08:00`, end: `2026-10-20T${Number(hour) + 1}:00+08:00`, time_zone: "Asia/Shanghai" },
    location: "教学楼" });
  await flow.intakeRecognizedMaterial({ descriptor, materialKind: "other", subject: null, classId: null }, {
    recognize: async () => ({ events: [event("09"), event("14")], slots: [] }),
    issue: async (command) => { commands.push(command); return { receipt: { status: "accepted" }, data: {} }; },
  });
  const imported = commands.find((command) => command.command_type === "import_calendar").events;
  assert.equal(imported.length, 2);
  assert.notEqual(imported[0].event_id, imported[1].event_id);
});
