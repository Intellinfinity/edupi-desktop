...
const seriesRef = (uid) => `ics-series:${crypto.createHash("sha256").update(uid, "utf8").digest("hex")}`;

test("keeps DOCX dependency loading inside the packaged worker", () => {
  const source = fs.readFileSync(new URL("./edupi-material-recognition.ts", import.meta.url), "utf8");
  const worker = fs.readFileSync(new URL("../desktop/docx-text-worker.cjs", import.meta.url), "utf8");
  assert.match(source, /docx-text-worker\.cjs/);
  assert.match(worker, /require\("mammoth"\)/);
  assert.doesNotMatch(source, /createRequire|import .*mammoth|require\("mammoth"\)/u);
});
...
test("accepts typed time and location only when an exact normalized text excerpt proves them", () => {
  const source = "通知：2026年10月20日 09:00-10:00（北京时间）在东楼 203 举行教研会。";
  const result = recognition.parseRecognitionOutput(JSON.stringify({ events: [{
    date: "2026-10-20", name: "教研会", type: "meeting", notes: null, evidence_quote: source,
    time_interval: { start: "2026-10-20T09:00+08:00", end: "2026-10-20T10:00+08:00", time_zone: "Asia/Shanghai" },
    location: "东楼 203",
  }], slots: [] }), () => "text-evidence-1", source);
  assert.deepEqual(result.events, [{
    event_id: "text-evidence-1", date: "2026-10-20", end_date: null, name: "教研会", type: "meeting",
...
test("rejects model-invented occurrence identity and typed details without source-bound evidence", () => {
  const validSource = "2026年10月1日 09:00-10:00（北京时间），地点：东楼 203，举行教研会。";
  for (const [event, source] of [
    [{ date: "2026-10-01", name: "无摘录", type: "meeting", time_interval: { start: "2026-10-01T09:00+08:00", end: "2026-10-01T10:00+08:00", time_zone: "Asia/Shanghai" } }, validSource],
    [{ date: "2026-10-01", name: "摘录不在原文", type: "meeting", evidence_quote: "另一份材料 09:00-10:00 北京时间", time_interval: { start: "2026-10-01T09:00+08:00", end: "2026-10-01T10:00+08:00", time_zone: "Asia/Shanghai" } }, validSource],
    [{ date: "2026-10-01", name: "时间不符", type: "meeting", evidence_quote: validSource, time_interval: { start: "2026-10-01T14:00+08:00", end: "2026-10-01T15:00+08:00", time_zone: "Asia/Shanghai" } }, validSource],
    [{ date: "2026-10-01", name: "时区无证据", type: "meeting", evidence_quote: "2026年10月1日 09:00-10:00", time_interval: { start: "2026-10-01T09:00+08:00", end: "2026-10-01T10:00+08:00", time_zone: "Asia/Shanghai" } }, "2026年10月1日 09:00-10:00"],
    [{ date: "2026-10-01", name: "地点不符", type: "meeting", evidence_quote: validSource, location: "西楼 101" }, validSource],
    [{ date: "2026-10-01", name: "家长会", type: "meeting", evidence_quote: validSource, location: "东楼 203" }, validSource],
    [{ date: "2026-10-01", name: "教研会", type: "meeting", evidence_quote: validSource, location: "东楼 203" }, validSource],
    [{ date: "2026-10-01", name: "教研会", type: "meeting", evidence_quote: validSource,
      time_interval: { start: "2026-10-01T09:00+08:00", end: "2026-10-01T10:00+08:00", time_zone: "Asia/Shanghai" } }, validSource],
    [{ date: "2026-10-01", end_date: "2026-10-02", name: "教研会", type: "meeting", evidence_quote: validSource,
      time_interval: { start: "2026-10-01T09:00+08:00", end: "2026-10-01T10:00+08:00", time_zone: "Asia/Shanghai" }, location: "东楼 203" }, validSource],
    [{ date: "2026-10-01", name: "教研会", type: "meeting",
      evidence_quote: "2026年10月1日 09:00-10:00 UTC+5 在东楼举行教研会。",
      time_interval: { start: "2026-10-01T09:00+05:30", end: "2026-10-01T10:00+05:30", time_zone: "Asia/Kolkata" }, location: "东楼" },
    "2026年10月1日 09:00-10:00 UTC+5 在东楼举行教研会。"],
    [{ date: "2026-10-01", name: "教研会", type: "meeting",
      evidence_quote: "2026-10-010 09:00-10:00 +08:00 在东楼举行教研会。",
      time_interval: { start: "2026-10-01T09:00+08:00", end: "2026-10-01T10:00+08:00", time_zone: "Asia/Shanghai" }, location: "东楼" },
    "2026-10-010 09:00-10:00 +08:00 在东楼举行教研会。"],
    [{ date: "2026-10-01", name: "教研会", type: "meeting",
      evidence_quote: "2026年10月1日 09:00-10:00 UTC+5 在东楼举行教研会。",
      time_interval: { start: "2026-10-01T09:00+00:00", end: "2026-10-01T10:00+00:00", time_zone: "UTC" }, location: "东楼" },
    "2026年10月1日 09:00-10:00 UTC+5 在东楼举行教研会。"],
    [{ date: "2026-10-01", name: "教研会", type: "meeting",
      evidence_quote: "2026年10月1日 09:00-10:00 UTC +5 在东楼举行教研会。",
      time_interval: { start: "2026-10-01T09:00+00:00", end: "2026-10-01T10:00+00:00", time_zone: "UTC" }, location: "东楼" },
    ...
    "2026年10月1日 09:00-10:00 UTC（+5）在东楼举行教研会。"],
    [{ date: "2026-10-01", name: "教研会", type: "meeting",
      evidence_quote: "2026年10月1日 09:00-10:00 北京时间 UTC+9 在东楼举行教研会。",
      time_interval: { start: "2026-10-01T09:00+08:00", end: "2026-10-01T10:00+08:00", time_zone: "Asia/Shanghai" }, location: "东楼" },
    "2026年10月1日 09:00-10:00 北京时间 UTC+9 在东楼举行教研会。"],
    ...["UTC−5", "UTC −5", "UTC（−5）"].map((zoneText) => [{ date: "2026-10-01", name: "教研会", type: "meeting",
      evidence_quote: `2026年10月1日 09:00-10:00 ${zoneText} 在东楼举行教研会。`,
      time_interval: { start: "2026-10-01T09:00+00:00", end: "2026-10-01T10:00+00:00", time_zone: "UTC" }, location: "东楼" },
    `2026年10月1日 09:00-10:00 ${zoneText} 在东楼举行教研会。`]),
    [{ date: "2026-10-01", name: "教研会", type: "meeting",
      evidence_quote: "2026年10月1日 09:00 Australia/Brisbane +10:00 在东楼举行教研会。",
      time_interval: { start: "2026-10-01T09:00+10:00", end: "2026-10-01T10:00+10:00", time_zone: "Australia/Brisbane" }, location: "东楼" },
    "2026年10月1日 09:00 Australia/Brisbane +10:00 在东楼举行教研会。"],
    [{ date: "2026-10-01", name: "视觉来源不可证", type: "meeting", evidence_quote: "09:00 北京时间 东楼", location: "东楼" }, ""],
    [{ date: "2026-10-01", name: "来源错误", type: "meeting", source_occurrence_ref: "model-invented" }, validSource],
  ]) {
    assert.throws(() => recognition.parseRecognitionOutput(JSON.stringify({ events: [event], slots: [] }), undefined, source),
      (error) => error?.code === "invalid_output");
  }
});

test("rejects a DST fold even with an explicit offset because the pinned Core contract is fail-closed", () => {
  const ambiguous = "2026年11月1日 01:30-01:45 America/New_York 在东楼举行教研会。";
  const event = { date: "2026-11-01", name: "教研会", type: "meeting", evidence_quote: ambiguous,
    time_interval: { start: "2026-11-01T01:30-04:00", end: "2026-11-01T01:45-04:00", time_zone: "America/New_York" }, location: "东楼" };
  assert.throws(() => recognition.parseRecognitionOutput(JSON.stringify({ events: [event], slots: [] }), undefined, ambiguous),
    (error) => error?.code === "invalid_output");
  const proved = ambiguous.replace("America/New_York", "America/New_York -04:00");
  assert.throws(() => recognition.parseRecognitionOutput(JSON.stringify({ events: [{ ...event, evidence_quote: proved }], slots: [] }), undefined, proved),
    (error) => error?.code === "invalid_output");
});

test("accepts a source-proved same-day interval crossing a DST offset transition", () => {
  const source = "2026年11月1日 00:30-02:30 America/New_York 在东楼举行教研会。";
  const result = recognition.parseRecognitionOutput(JSON.stringify({ events: [{
    date: "2026-11-01", name: "教研会", type: "meeting", evidence_quote: source,
    time_interval: { start: "2026-11-01T00:30-04:00", end: "2026-11-01T02:30-05:00", time_zone: "America/New_York" }, location: "东楼",
  }], slots: [] }), () => "dst-transition", source);
  assert.equal(result.events[0].event_id, "dst-transition");
  assert.equal(result.events[0].time_interval.end, "2026-11-01T02:30-05:00");
});

test("does not mistake a spaced time-range hyphen for a negative UTC offset", () => {
  const source = "2026年10月1日 09:00 - 10:00 北京时间 在东楼举行教研会。";
  const result = recognition.parseRecognitionOutput(JSON.stringify({ events: [{
    date: "2026-10-01", name: "教研会", type: "meeting", evidence_quote: source,
    time_interval: { start: "2026-10-01T09:00+08:00", end: "2026-10-01T10:00+08:00", time_zone: "Asia/Shanghai" }, location: "东楼",
  }], slots: [] }), () => "spaced-range", source);
  assert.equal(result.events[0].event_id, "spaced-range");
});

test("normalizes a Unicode minus when it proves the actual UTC offset", () => {
  const source = "2026年10月1日 09:00-10:00 UTC−5 在东楼举行教研会。";
  const result = recognition.parseRecognitionOutput(JSON.stringify({ events: [{
    date: "2026-10-01", name: "教研会", type: "meeting", evidence_quote: source,
    time_interval: { start: "2026-10-01T09:00-05:00", end: "2026-10-01T10:00-05:00", time_zone: "America/Bogota" }, location: "东楼",
  }], slots: [] }), () => "unicode-minus", source);
  assert.equal(result.events[0].event_id, "unicode-minus");
});

test("normalizes model UTC Z instants to the offset form required by Core", () => {
  const source = "2026年10月1日 09:00-10:00 UTC 在东楼举行教研会。";
  const result = recognition.parseRecognitionOutput(JSON.stringify({ events: [{
    date: "2026-10-01", name: "教研会", type: "meeting", evidence_quote: source,
    time_interval: { start: "2026-10-01T09:00Z", end: "2026-10-01T10:00Z", time_zone: "UTC" }, location: "东楼",
  }], slots: [] }), () => "utc-z", source);
  assert.deepEqual(result.events[0].time_interval,
    { start: "2026-10-01T09:00+00:00", end: "2026-10-01T10:00+00:00", time_zone: "UTC" });
});
...
test("parses explicit ICS identities, timezones, all-day ranges, and finite recurrence", async () => {
  ...
  assert.deepEqual(first.cancelled_occurrence_refs, [`${seriesRef("weekly-1")}#2026-09-08T06:00:00.000Z`],
...
test("uses the original recurrence key for moved overrides and keeps cancellations out of active events", async () => {
  ...
  assert.deepEqual(result.cancelled_occurrence_refs, undefined);
  assert.deepEqual(result.affected_series_refs, [seriesRef("cancelled-1")]);
...
test("parses a standalone RECURRENCE-ID cancellation without requiring the master RRULE", async () => {
  ...
  assert.deepEqual(result.events, []);
  assert.deepEqual(result.cancelled_occurrence_refs, [`${seriesRef("series-cancel")}#2026-10-27T01:00:00.000Z`]);
});

test("marks a master cancellation without DTSTART as affecting the whole UID series", async () => {
  const bytes = Buffer.from([
    "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//EduPi Test//EN", "METHOD:CANCEL",
    "BEGIN:VEVENT", "UID:series-cancel-all", "DTSTAMP:20260901T000000Z",
    "STATUS:CANCELLED", "END:VEVENT", "END:VCALENDAR", "",
  ].join("\r\n"));
  const result = await recognition.parseIcsCalendar(bytes);
  assert.equal(result.calendar_mode, "delta_cancel");
  assert.deepEqual(result.events, []);
  assert.equal(result.cancelled_occurrence_refs, undefined);
  assert.deepEqual(result.affected_series_refs, [seriesRef("series-cancel-all")]);
});
...
test("distinguishes full snapshots, delta upserts, and cancellation deltas", async () => {
  ...
  assert.deepEqual(cancelled.cancelled_occurrence_refs, undefined);
  assert.deepEqual(cancelled.affected_series_refs, [seriesRef("CANCEL")]);
});

test("marks a REQUEST recurring master as replacing only its bounded UID series", async () => {
  const bytes = Buffer.from([
    "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//EduPi Test//EN", "METHOD:REQUEST",
    "BEGIN:VEVENT", "UID:series-replace", "DTSTAMP:20260901T000000Z",
    "DTSTART;TZID=Asia/Shanghai:20261020T100000", "DTEND;TZID=Asia/Shanghai:20261020T110000",
    "RRULE:FREQ=WEEKLY;COUNT=3", "SUMMARY:改时循环事项", "END:VEVENT", "END:VCALENDAR", "",
  ].join("\r\n"));
  const result = await recognition.parseIcsCalendar(bytes);
  assert.equal(result.calendar_mode, "delta_upsert");
  assert.deepEqual(result.affected_series_refs, [seriesRef("series-replace")]);
  assert.equal(result.events.length, 3);
});

test("marks REQUEST recurring-to-single changes with the same UID series family", async () => {
  const bytes = Buffer.from([
    "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//EduPi Test//EN", "METHOD:REQUEST",
    "BEGIN:VEVENT", "UID:series-to-single", "DTSTAMP:20260901T000000Z",
    "DTSTART;TZID=Asia/Shanghai:20261020T100000", "DTEND;TZID=Asia/Shanghai:20261020T110000",
    "SUMMARY:改为单次事项", "END:VEVENT", "END:VCALENDAR", "",
  ].join("\r\n"));
  const result = await recognition.parseIcsCalendar(bytes);
  assert.deepEqual(result.affected_series_refs, [seriesRef("series-to-single")]);
  assert.equal(result.events[0].source_occurrence_ref, "series-to-single");
});

test("reserves generated ICS identity namespaces from non-recurring UIDs", async () => {
  const rawUid = `ics-series:${"a".repeat(64)}#2026-10-20T01:00:00.000Z`;
  const bytes = Buffer.from([
    "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//EduPi Test//EN",
    "BEGIN:VEVENT", `UID:${rawUid}`, "DTSTAMP:20260901T000000Z",
    "DTSTART;TZID=Asia/Shanghai:20261020T100000", "DTEND;TZID=Asia/Shanghai:20261020T110000",
    "SUMMARY:非循环保留命名空间", "END:VEVENT", "END:VCALENDAR", "",
  ].join("\r\n"));
  const result = await recognition.parseIcsCalendar(bytes);
  assert.match(result.events[0].source_occurrence_ref, /^ics:[a-f0-9]{64}$/u);
  assert.notEqual(result.events[0].source_occurrence_ref, rawUid);
});
...
test("passes validated text evidence through recognition as inferred typed schedule details", async () => {
  const source = "2026年9月2日 14:00-15:30（北京时间），地点：报告厅，举行教师大会。";
  const result = await recognition.recognizeStagedMaterial(descriptor, {
    extract: async () => ({ text: source, images: [] }),
    runModel: async () => JSON.stringify({ events: [{ date: "2026-09-02", name: "教师大会", type: "meeting",
      evidence_quote: source, time_interval: { start: "2026-09-02T14:00+08:00", end: "2026-09-02T15:30+08:00", time_zone: "Asia/Shanghai" },
      location: "报告厅" }], slots: [] }),
    idFactory: () => "typed-text-event",
  });
  assert.equal(result.events[0].confidence, "inferred");
  assert.deepEqual(result.events[0].time_interval,
    { start: "2026-09-02T14:00+08:00", end: "2026-09-02T15:30+08:00", time_zone: "Asia/Shanghai" });
  assert.equal(result.events[0].location, "报告厅");
  assert.equal(JSON.stringify(result).includes("evidence_quote"), false);
});

test("legacy DOC text cannot opt into the PDF and DOCX typed evidence contract", async () => {
  const source = "2026年9月2日 14:00-15:30（北京时间），地点：报告厅，举行教师大会。";
  await assert.rejects(recognition.recognizeStagedMaterial({ ...descriptor, kind: "word", original_name: "旧通知.doc",
    staging_path: descriptor.staging_path.replace(/\.pdf$/u, ".doc") }, {
    extract: async () => ({ text: source, images: [] }),
    runModel: async () => JSON.stringify({ events: [{ date: "2026-09-02", name: "教师大会", type: "meeting",
      evidence_quote: source, time_interval: { start: "2026-09-02T14:00+08:00", end: "2026-09-02T15:30+08:00", time_zone: "Asia/Shanghai" },
      location: "报告厅" }], slots: [] }),
...
test("runs DOCX and PDF extraction in bounded child processes instead of the server process", () => {
  const source = fs.readFileSync(new URL("./edupi-material-recognition.ts", import.meta.url), "utf8");
  const pdfHelper = fs.readFileSync(new URL("../desktop/pdf-page-extract.mjs", import.meta.url), "utf8");
  ...
  assert.match(source, /pdf-page-extract\.mjs/);
  assert.match(source, /pageCount > MAX_MODEL_IMAGES/);
  assert.match(pdfHelper, /pdf\.numPages > MAX_SCAN_PAGES/);
  assert.ok(pdfHelper.indexOf("pdf.numPages > MAX_SCAN_PAGES") < pdfHelper.indexOf("page.render"), "page count is gated before partial OCR");
});
...
test("pins recognition bytes to the private staging root and rejects swaps or symlinks", async () => {
  ...
  try {
    ...
    assert.throws(() => recognition.saveRecognitionCache(boundaryDescriptor, { events: [{ ...cachedResult.events[0],
      time_interval: { start: "2026-09-01T09:00+08:00", end: "2026-09-01T10:00+08:00", time_zone: "Asia/Shanghai" },
      location: "报告厅" }], slots: [] }), (error) => error?.code === "invalid_output",
    "non-ICS typed details must never load from a cache that cannot re-prove the source excerpt");
    const cacheFile = path.join(directory, "recognition-result.json");
    const typedSource = "2026年9月1日 09:00-10:00（北京时间），在报告厅举行开学会。";
    const typedOutput = JSON.stringify({ events: [{ date: "2026-09-01", name: "开学会", type: "meeting", evidence_quote: typedSource,
      time_interval: { start: "2026-09-01T09:00+08:00", end: "2026-09-01T10:00+08:00", time_zone: "Asia/Shanghai" }, location: "报告厅" }], slots: [] });
    const typedResult = recognition.parseRecognitionOutput(typedOutput, () => "typed-cache-event", typedSource);
    const textDescriptor = { ...boundaryDescriptor, original_name: "校历.pdf", kind: "pdf",
      staging_path: boundaryDescriptor.staging_path.replace(/\.png$/u, ".pdf") };
    recognition.saveRecognitionCache(textDescriptor, typedResult, { modelOutput: typedOutput, sourceText: typedSource });
    assert.deepEqual(recognition.loadRecognitionCache(textDescriptor, typedSource), typedResult,
      "typed cache replay must re-parse its source-bound model output");
    const typedCache = JSON.parse(fs.readFileSync(cacheFile, "utf8"));
    typedCache.model_output = typedCache.model_output.replace("报告厅", "不存在的西楼");
    fs.writeFileSync(cacheFile, `${JSON.stringify(typedCache)}\n`);
    assert.throws(() => recognition.loadRecognitionCache(textDescriptor, typedSource), (error) => error?.code === "extract_unavailable");
    assert.throws(() => recognition.saveRecognitionCache({ ...textDescriptor, kind: "word", original_name: "旧校历.doc",
      staging_path: textDescriptor.staging_path.replace(/\.png$/u, ".doc") }, typedResult,
    { modelOutput: typedOutput, sourceText: typedSource }), (error) => error?.code === "invalid_output");
    recognition.saveRecognitionCache(boundaryDescriptor, cachedResult);
    const legacyCache = JSON.parse(fs.readFileSync(cacheFile, "utf8"));
    const tamperedCache = structuredClone(legacyCache);
    tamperedCache.result.events[0].time_interval = { start: "2026-09-01T09:00+08:00", end: "2026-09-01T10:00+08:00", time_zone: "Asia/Shanghai" };
    fs.writeFileSync(cacheFile, `${JSON.stringify(tamperedCache)}\n`);
    assert.throws(() => recognition.loadRecognitionCache(boundaryDescriptor), (error) => error?.code === "extract_unavailable");
