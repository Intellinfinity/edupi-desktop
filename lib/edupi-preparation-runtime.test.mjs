...
function load(host, data, connected = true, pump = async () => {}, kernelRuns = []) {
  const dependencies = {
    ...
    readEducationContract: async () => { if (data instanceof Error) throw data; return data; },
    pumpBackgroundJobs: pump,
    readEduPiKernelProjection: async () => ({ projection: { runs: kernelRuns } }),
  };
...
test("an explicit Core preflight failure survives a Desktop status reload", async () => {
  const task = { id: "task-preflight", revision: 2 };
  const api = load({}, { tasks: [task], workCases: [{ taskId: task.id, currentState: "planned", artifactIds: [] }] }, false, async () => {}, [{ trigger_id: "g1_prepare_task", fire_key: `task:${task.id}:r2`, status: "failed", error_code: "excerpt_unconfirmed" }]);
  const result = await api.preparationStatus(task.id);
  assert.equal(result.state, "error");
  assert.equal(result.error, "请先确认材料内容");
});

test("a transient status read failure remains retryable", async () => {
  const api = load({}, new Error("temporary read failure"), true);
  const result = await api.preparationStatus("task-1");
...
test("runtime startup immediately scans due Core work and resumes managed background jobs", async () => {
  let pumps = 0;
  const calls = [];
  const api = load({ call: async (operation, payload) => {
    calls.push([operation, payload]);
    return { ok: true, result: operation === "health" ? { queue: { queued: 0, claimed: 0 } } : { tasks: [], failures: [] } };
  } }, { workCases: [] }, true, async () => { pumps++; });
  await api.ensurePreparation();
  assert.equal(pumps, 1);
  assert.deepEqual(calls.map(([operation]) => operation), ["prepare_due", "health"]);
});

test("a failed preparation scan does not block independent background recovery", async () => {
  let pumps = 0;
  const api = load({ call: async () => ({ ok: false, error_code: "invalid_state" }) }, { workCases: [] }, true, async () => { pumps++; });
  const result = await api.ensurePreparation();
  assert.equal(result.state, "error");
  assert.equal(pumps, 1);
});
