...
test("supervisor shares startup, binds private requests, rejects mismatches and clears failed children", async () => {
  ...
  let requests = 0, wrongBinding = false, privateToken, ownerControlHeader, wrongReady = false, startupErrorCode = null, forks = 0, stopped = 0;
  ...
  const modules = {
    ...
    "./edupi-owner-control-token": { loadOwnerControlToken: () => "test-persistent-owner-token" },
  };
  ...
  try {
    ...
    await assert.rejects(handle.callOwnerControl("health", null), { code: "runtime_unavailable" });
