...
test("same occurrence moves cannot be kept as two events", () => {
  const interval = { start: "2026-10-12T09:00+08:00", end: "2026-10-12T10:00+08:00", time_zone: "Asia/Shanghai" };
  const occurrence = conflicts.normalizeScheduleConflict({ ...conflict,
    canonical: { ...conflict.canonical, source_occurrence_ref: "meeting-42", time_interval: interval, location: "东楼" },
    candidate: { ...conflict.candidate, source_occurrence_ref: "meeting-42", time_interval: {
      start: "2026-10-13T11:00+08:00", end: "2026-10-13T12:00+08:00", time_zone: "Asia/Shanghai" }, location: "西楼" },
  });
  assert.equal(occurrence.canonical.location, "东楼");
  assert.throws(() => conflicts.captureScheduleDecision(occurrence, "keep_both_distinct", "desktop-same-occurrence"),
    (error) => error?.code === "invalid_conflict_request");
  assert.equal(conflicts.captureScheduleDecision(occurrence, "replace_with_candidate", "desktop-move-occurrence").decision, "replace_with_candidate");
  assert.throws(() => conflicts.normalizeScheduleConflict({ ...conflict,
    canonical: { ...conflict.canonical, source_occurrence_ref: "meeting-42", time_interval: { ...interval, start: "09:00" } },
  }), (error) => error?.code === "invalid_conflict_response");
});
