...
test("bootstraps an inactive owner before binding the target and recording", async () => {
  ...
  const responses = [
    ...
    new Response(JSON.stringify({ ok: true, result: { kind: "work_candidate", target_id: "candidate-1", revision: 2, fingerprint: `sha256:${"a".repeat(64)}`, evidence_ids: ["evidence-1"] } }), { status: 200 }),
...
test("does not attribute a rating to a candidate revised after review", async () => {
  const targetRead = new Response(JSON.stringify({ ok: true, result: { kind: "work_candidate", target_id: "candidate-1", revision: 3, fingerprint: `sha256:${"a".repeat(64)}`, evidence_ids: ["evidence-1"] } }), { status: 200 });
