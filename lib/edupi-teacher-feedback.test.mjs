...
test("captures real teacher usage and paired time measurements", () => {
  const record = feedback.buildTeacherFeedbackRecord({
    ...input,
    used: true,
    wouldUseAgain: true,
    baselineMinutes: 30,
  ...
  assert.equal(record.would_use_again, true);
  assert.equal(record.baseline_minutes, 30);
  assert.equal(record.review_minutes, 6);
  assert.equal(record.note, "实际用于七一班。");
  for (const change of [
    { baselineMinutes: 30, reviewMinutes: null },
    { baselineMinutes: null, reviewMinutes: 6 },
    { baselineMinutes: 0, reviewMinutes: 6 },
    { baselineMinutes: 30, reviewMinutes: 1441 },
    { decision: "reject", used: true },
  ]) {
    assert.throws(() => feedback.buildTeacherFeedbackRecord({ ...input, ...change }), (error) => error?.code === "invalid_feedback");
  }
});

test("builds a real-teacher missed-opportunity record without inventing a current target", () => {
  const capture = feedback.createMissedTeacherFeedbackCapture({
    domain: "calendar_administration",
    scope: { classId: "class-7b", subject: "administration" },
    note: "系统没有提醒本周五前提交材料。",
    occurredAt: "2026-09-22T01:02:03.000Z",
    randomId: "01234567-89ab-4cde-8fab-0123456789ab",
  });
  const record = feedback.buildTeacherFeedbackRecord(capture);
  assert.equal(record.signal, "missed");
  assert.equal(record.decision, "missed");
  assert.equal(record.usefulness, "not_observed");
  assert.equal(record.evidence_level, "real_teacher");
  assert.deepEqual(record.target, {
    kind: "missed_opportunity",
    target_id: "missed_opportunity:01234567-89ab-4cde-8fab-0123456789ab",
    expected_revision: 0,
    expected_fingerprint: null,
  });
  assert.deepEqual(record.evidence_ids, ["teacher_report:01234567-89ab-4cde-8fab-0123456789ab"]);
  assert.throws(() => feedback.createMissedTeacherFeedbackCapture({
    domain: "calendar_administration",
    scope: { classId: "class-7b", subject: "administration" },
    note: " ",
  }), (error) => error?.code === "invalid_feedback");
});
