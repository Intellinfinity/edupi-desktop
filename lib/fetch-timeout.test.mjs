...
test("a response whose body never finishes also reaches the deadline", async () => {
  const { fetchJsonWithDeadline } = await loadSubject();
  let aborted = false;
  ...
  await assert.rejects(fetchJsonWithDeadline("/api/mobile/sessions", 40), /timed out after 40ms/);
  assert.equal(aborted, true);
});

test("a complete JSON response returns the status and parsed body", async () => {
  const { fetchJsonWithDeadline } = await loadSubject();
  globalThis.fetch = () => Promise.resolve({ ok: true, status: 200, json: async () => ({ sessions: [] }) });
  const { response, body } = await fetchJsonWithDeadline("/api/mobile/sessions", 40);
