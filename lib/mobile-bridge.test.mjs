...
test("only the same pairing attempt can recover a lost request or activation response", () => {
  ...
  const activated = bridge.completeMobilePairing(created.id, created.code);
  assert.equal(bridge.completeMobilePairing(created.id, created.code).token, activated.token);
