...
test("EduPi native-tool gates accept the canonical data or Core project root", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  assert.match(source, /const sessionIsEduPiDataRoot = normalizedSessionCwd === EDUPI_ROOT/);
  assert.match(source, /const sessionIsEduPiCoreRoot = normalizedSessionCwd === EDUPI_CODE_ROOT/);
  assert.match(source, /createMemoryWriteTool\(EDUPI_CODE_ROOT\)/);
  assert.match(source, /createEduPiTaskTool\(\{ projectRoot: EDUPI_CODE_ROOT \}\)/);
});
