...
test("both proxy commands are scoped to the trusted desktop WebView", async () => {
  ...
  assert.match(shell, /file\.sync_all\(\)[\s\S]*replace_file_atomically\(&temporary, path\)/);
});

test("Windows replaces an existing proxy file atomically", async () => {
  const [shell, cargo] = await Promise.all([
    readFile(new URL("../src-tauri/src/lib.rs", import.meta.url), "utf8"),
    readFile(new URL("../src-tauri/Cargo.toml", import.meta.url), "utf8"),
  ]);
  assert.match(cargo, /windows-sys = \{[^\n]*features = \["Win32_Storage_FileSystem"\]/u);
  assert.match(shell, /#\[cfg\(windows\)\][\s\S]*fn replace_file_atomically\(/u);
  assert.match(shell, /MoveFileExW\([\s\S]*MOVEFILE_REPLACE_EXISTING \| MOVEFILE_WRITE_THROUGH/u);
  assert.match(shell, /replace_file_atomically\(&temporary, path\)/u);
});
