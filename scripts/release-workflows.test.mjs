...
test("the manifest job only publishes when every platform succeeded", async () => {
  ...
  assert.match(manifestJob, /repos\/\$RELEASE_REPOSITORY\/git\/blobs/);
  assert.match(manifestJob, /repos\/\$RELEASE_REPOSITORY\/git\/trees/);
  assert.match(manifestJob, /parents: \[\$parent\]/);
  assert.match(manifestJob, /parents: \[\]/);
  assert.match(manifestJob, /force=false/);
  assert.doesNotMatch(manifestJob, /contents\/latest\.json/);
  ...
  assert.match(manifestJob, /FEED_BRANCH: updater-feed/);
  assert.match(manifestJob, /raw\.githubusercontent\.com\/\$RELEASE_REPOSITORY\/\$FEED_BRANCH\/latest\.json\?commit=\$feed_commit/);
...
test("release verifies paired runtime where supported and exact bundle bytes on Windows", async () => {
  ...
  assert.match(buildJob, /name: Verify staged packaged server[\s\S]*?run: npm run test:staged-desktop-runtime/);
  assert.ok(buildJob.includes("EDUPI_STAGED_IDENTITY_ONLY"));
...
test("every packaged workflow checks out the exact pinned Core runtime", async () => {
  ...
  for (const workflow of workflows) {
    ...
    assert.match(workflow, /git -C \.edupi-core-runtime archive HEAD \| tar -x -C \.edupi-core-runtime/);
